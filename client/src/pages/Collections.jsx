    ...
    <div className="max-w-4xl mx-auto px-6 md:px-10 py-8 fade-rise">
      <div className="flex items-center justify-between mb-8">
        <div>
          ...
          <p className="text-sm text-fog mt-1">Your collections</p>
        </div>
        <button
      ...
      {creating && (
        <form onSubmit={handleCreate} className="surface rounded-2xl p-5 mb-6 space-y-3">
          ...
          <label className="flex items-center gap-2.5 cursor-pointer">
            ...
            />
            <span className="text-sm text-dim">Public</span>
          </label>
          <div className="flex gap-2 pt-1">
            <button type="submit" className="btn-primary rounded-full px-4 py-2">
              Create
            </button>
            <button
              type="button"
              onClick={() => setCreating(false)}
              className="px-4 py-2 rounded-full text-sm font-medium text-fog hover:text-ink transition-colors"
            >
              Cancel
            </button>
          </div>
